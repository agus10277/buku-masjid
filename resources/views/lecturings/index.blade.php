...
<div class="card">
    <div class="card-body">
        {{ Form::open(['method' => 'get', 'class' => 'form-inline']) }}
            ...
            {{ Form::select('month', get_months(), $month, ['class' => 'form-control mr-0 mr-sm-6']) }}
            {{ Form::select('year', get_years(), $year, ['class' => 'form-control mr-0 mr-sm-6']) }}
            <div class="form-group mt-4 mt-sm-0">
                {{ Form::submit(__('app.submit'), ['class' => 'btn btn-primary mr-0 mr-sm-6']) }}
                {{ link_to_route('lecturings.index', __('app.reset'), [], ['class' => 'btn btn-secondary mr-0 mr-sm-6']) }}
                @livewire('prev-month-button', ['routeName' => 'lecturings.index', 'buttonClass' => 'btn btn-secondary mr-0 mr-sm-6'])
                @livewire('next-month-button', ['routeName' => 'lecturings.index', 'buttonClass' => 'btn btn-secondary mr-0 mr-sm-6'])
            </div>
            <div class="form-group mt-0 mt-sm-0">
                @can('create', new App\Models\Lecturing)
                    {{ link_to_route('lecturings.create', __('lecturing.create'), [], ['class' => 'btn btn-success mr-0 mr-sm-5']) }}
